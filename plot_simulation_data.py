...
# Plot_Grid_TO_gamma( sim_data_sets, output_dir, plot_splines=True, system=system )


Plot_Grid_tau_HeII( sim_data_sets, output_dir, plot_splines=True, system=system )
...
nrows = 1
ncols = 3
fig, ax_l = plt.subplots(nrows=nrows, ncols=ncols, figsize=(ncols*6,7))
plt.subplots_adjust( hspace = 0.02, wspace=0.02)

for z_id, current_z in enumerate( z_vals ):
  ...
  x_min, x_max = 1.5e-3, 3e-2
  y_min, y_max = 1e-2, 2e-1
  ...
  if z_id ==0: ax.set_ylabel( r' $\Delta_F^2(k)$', fontsize=label_size, color= text_color )
