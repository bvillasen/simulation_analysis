...
comparable_data_tau = Get_Comparable_Tau()
comparable_grid_tau = Get_Comparable_Tau_from_Grid( comparable_data_tau, SG )
...
param_vals = SG.parameters[param_to_fit]['values']

nIter = 100000 
nBurn = nIter / 5
nThin = 1
model = mcmc_model_1D( param_to_fit, comparable_data_tau, comparable_grid_tau, SG )
MDL = pymc.MCMC( model )  
...
for observable in observables_to_sample:
  ...
  for i in range ( n_samples ):
    ...
    if param_val < min(param_vals): param_val = min(param_vals)
    if param_val > max(param_vals): param_val = max(param_vals)
...
output_dir = root_dir + 'fit_results_tau/'
...
for plot_type in ['grid', 'sampling']:
  ...
  ax.scatter( data_z, data_mean, label='Data', alpha=0.8, color= data_color, zorder=2) 
  ...
  figure_name = output_dir + f'fig_tau_{param_name}_{plot_type}.png'
