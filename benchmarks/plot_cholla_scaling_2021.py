...
data = np.array([                        # dt    hydo     bound  grav_pot  pot_bou  part_dens   part_bd part_ds_  part_adv_1  part_adv_2  total
  ...
  [ 128, 4096, 2048, 2048, 2048, 7, 25, 0.53560, 14.3434, 23.0651, 77.719, 3.27275, 0.260162, 2.28842, 0.817766, 0.017643, 0.0526905, 122.373 ],  
  [ 128, 8192, 4096, 2048, 2848, 7, 25, 0.5299,  14.3556, 21.3827, 80.1377, 2.8046,  0.2674,  2.3525,  0.7066, 0.025767,  0.054232, 122.6170   ],
  [ 128, 16384, 4096, 4096, 2048, 7, 25, 0.570517, 14.3627, 22.7083, 83.8394, 2.87038, 0.271807, 2.3234, 0.74935, 0.0253487, 0.0618553, 127.783 ]
]).T
...
t_grav_128_logN = t_grav_128 / np.log( N_128 )
t_total_128_logN = t_total_128 - t_grav_128 + t_grav_128_logN 
...
t_grav_256_logN = t_grav_256 / np.log( N_256 )
t_total_256_logN = t_total_256 - t_grav_256 + t_grav_256_logN
...
if plot_grav_over_logN: 
  ax.plot( n_procs_128, t_grav_128_logN, '--', c=c_grav, alpha=0.8, marker='o', markersize=ms)
  ax.plot( n_procs_128, t_total_128_logN, '--', c=c_total,  alpha=0.8, marker='o', markersize=ms, label=r'128$^3$ / GPU')
...
if plot_grav_over_logN: 
  ax.plot( n_procs_256, t_grav_256_logN, c=c_grav, alpha=0.6, marker='D', markersize=ms)
  ax.plot( n_procs_256, t_total_256_logN, c=c_total, alpha=0.6, marker='D', markersize=ms,  label=r'256$^3$ / GPU')
else: 
  ax.plot( n_procs_256, t_grav_256, c=c_grav, alpha=0.6, marker='D', markersize=ms)
  ax.plot( n_procs_256, t_total_256, c=c_total, alpha=0.6, marker='D', markersize=ms,  label=r'256$^3$ / GPU')
...
if plot_grav_over_logN: 
  ...
  ax.text(0.05, 0.7, 'Total', fontsize=fs, color=c_total, horizontalalignment='left', verticalalignment='center', transform=ax.transAxes)
...
if plot_grav_over_logN: fileName = output_dir + 'scaling_summit_adiabatic_2021_log_new_logN.png'
else: fileName = output_dir + 'scaling_summit_adiabatic_2021_log_new.png'
