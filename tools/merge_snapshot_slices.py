...
root_dir  = data_dir + f'cosmo_sims/sim_grid/1024_P19m_np4_nsim400/slices_params_{params_type}/'
output_dir = root_dir
...
if params_type == 'He': field = 'temperature'
if params_type == 'H':  field = 'HI_density'
...
for i in range( n_sims_y ):
  for j in range( n_sims_x ):
    indx = i * n_sims_x + j
    sim_id = sim_ids[indx]
    print( f'Ids: {i}  {j}   sim_id{sim_id}') 
    in_file_name  = root_dir  + f'sim_{sim_id}/slice_{n_snap}_start{slice_start}_depth{slice_depth}.h5'
    print( f'Loading File: {in_file_name }' )
    in_file = h5.File( in_file_name, 'r' )
    
    slice = in_file[field][...]
    slice_size = slice.shape
    in_file.close()
    
    if not have_slice_extended:
      size_extended = ( slice_size[0] , n_sims_y * slice_size[1], n_sims_x * slice_size[2] )
      print( f' Extended Size: {size_extended}'  )
      slice_extended = np.zeros( size_extended )
      have_slice_extended = True
...
#   if params_type == 'H':
#     scale_H  = sim_data['parameters']['scale_H']
#     deltaZ_H = sim_data['parameters']['deltaZ_H']
#     param_vals_x.append( scale_H )
#     param_vals_y.append( deltaZ_H )
#   in_file_name = input_dir + f'slice_{n_snap}_start{slice_start}_depth{slice_depth}.h5'
#   in_file = h5.File( in_file_name,  'r' )
#   current_z = in_file.attrs['current_z']
#   slice = in_file[field][...] / scale_H
#   if extend:
...
#     id_lu = indices_2D[ indx_y+n_vals_y -1, indx_x   ]
#     id_rd = indices_2D[ indx_y,             indx_x+n_vals_x -1  ]
#     id_ru = indices_2D[ indx_y+n_vals_y -1, indx_x+n_vals_x -1 ] 
# 
